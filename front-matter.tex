...
A \emph{coordinate} is an element of a simply ordered set,\sidenote{%
...
which is mapped continuously and in a one-to-one way onto a path.\sidenote{%
   A function takes every coordinate to a corresponding point on the path.  For
   the function to be continuous, an infinitesimal change in coordinate must
   never produce a finite change in the distance along the path between mapped
   points.  The one-to-one nature of the mapping means that for every point
   along the path, there is exactly one coordinate corresponding to it.%
}
